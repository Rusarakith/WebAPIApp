...
const HotelEditPopup = ({ title, isOpen, onClose, data, onEdit }) => {
    const [hotelName, setHotelName] = useState(data[0].hotelName)
    const [destination, setDestination] = useState(data[0].destination);
    const [starRating, setStarRating] = useState(data[0].starRating);
    const [basePrice, setBasePrice] = useState(data[0].basePrice);
    const [hasPool, setHasPool] = useState(data[0].hasPool);
    const [hasKidsPlayArea, setHasKidsPlayArea] = useState(data[0].hasKidsPlayArea);
    const [hasGym, setHasGym] = useState(data[0].hasGym);
    const [hasBeachAccess, setHasBeachAccess] = useState(data[0].hasBeachAccess);
    const [deluxeFullBoardPrice, setDelixeFullBoardPrice] = useState(data[0].deluxeRoom.price.fullboardPrice);
    const [deluxeHalfBoardPrice, setDeluxeHalfBoardPrice] = useState(data[0].deluxeRoom.price.halfBoardPrice);
    const [deluxeBandB, setdeluxeBandB] = useState(data[0].deluxeRoom.price.bedAndBreakfastPrice);
    const [deluxeAvlRooms, setDeluxeAvlRoom] = useState(data[0].deluxeRoom.availableRooms);
    const [superDeluxeFullBoardPrice, setSuperDelixeFullBoardPrice] = useState(data[0].superDeluxeRoom.price.fullboardPrice);
    const [superDeluxeHalfBoardPrice, setSuperDelixeHalfBoardPrice] = useState(data[0].superDeluxeRoom.price.halfBoardPrice);
    const [superDeluxeBandB, setSuperDelixeBandB] = useState(data[0].superDeluxeRoom.price.bedAndBreakfastPrice);
    const [superDeluxeAvlRooms, setSuperDelixeAvlRoom] = useState(data[0].superDeluxeRoom.availableRooms);
    const [suiteFullBoardPrice, setSuiteFullBoardPrice] = useState(data[0].suiteRoom.price.fullboardPrice);
    const [suiteHalfBoardPrice, setSuiteHalfBoardPrice] = useState(data[0].suiteRoom.price.halfBoardPrice);
    const [suiteBandB, setSuiteBandB] = useState(data[0].suiteRoom.price.bedAndBreakfastPrice);
    const [suiteAvlRooms, setSuiteAvlRoom] = useState(data[0].suiteRoom.availableRooms);
    const [hotelIsActive, setHotelIsActive] = useState(data.isActive);
